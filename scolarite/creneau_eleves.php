	...
	foreach ($object->fields as $key => $val) {
		...
		if (!empty($arrayfields['t.'.$key]['checked'])) {
			...
			if ($key == 'status') {
				...
				$profs .= '<a href="' . DOL_URL_ROOT . '/user/card.php?id=' . $objProf1->rowid . '">' .👤. $objProf1->lastname.' '.$objProf1->firstname. '</a>'.'<br><br>';
				...
				if(isset($object->fk_prof_2))
				{
					...
					$profs .= '<a href="' . DOL_URL_ROOT . '/user/card.php?id=' . $objProf2->rowid . '">' .👤. $objProf2->lastname.' '.$objProf2->firstname. '</a>'.'<br><br>';
				}
				if(isset($object->fk_prof_3))
				{
					...
					$profs .= '<a href="' . DOL_URL_ROOT . '/user/card.php?id=' . $objProf3->rowid . '">' .👤. $objProf3->lastname.' '.$objProf3->firstname. '</a>'.'<br><br>';
				}
				...
				$infos_creneau .= $objJour->jour.' | '.$objheure->heure.'h'.$object->minutes_debut.'-'.$objheureFin->heure.'h'.$object->minutes_fin.' | '.$objDiminutif->diminutif.'-'.$objSalle->salle;
