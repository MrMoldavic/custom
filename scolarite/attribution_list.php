...
/**
 *   	\file       attribution_list.php
 *		\ingroup    scolarite
 *		\brief      List page for attribution
 */
...
require_once __DIR__.'/class/attribution.class.php';
...
$hookmanager->initHooks(array('attributionlist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Attribution';
	$objectlabel = 'Attribution';
...
//$help_url = "EN:Module_Attribution|FR:Module_Attribution_FR|ES:Módulo_Attribution";
$help_url = '';
$title = $langs->trans("Attributions");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/scolarite/attribution_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/scolarite/attribution_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAttributionRef";
$modelmail = "attribution";
$objecttmp = new Attribution($db);
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				if ($key == 'status') {
					...
					print '<a href="' . DOL_URL_ROOT . '/custom/scolarite/attribution_card.php?id=' . $object->id . '">' . $objCle->numero_cle . '</a>';
