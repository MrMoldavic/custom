...
/**
 *	Class to manage PDF template standard_creneau
 */
class pdf_standard_creneau extends ModelePDFCreneau
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		/*
		if (!empty($conf->global->MAIN_GENERATE_CRENEAU_WITH_PICTURE))
		{
		...
		*/
		...
		if ($conf->scolarite->dir_output.'/creneau') {
			$object->fetch_thirdparty();

			// Definition of $dir and $file
			if ($object->specimen) {
				$dir = $conf->scolarite->dir_output.'/creneau';
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->scolarite->dir_output.'/creneau/'.$objectref;
