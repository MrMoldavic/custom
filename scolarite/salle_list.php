...
require_once DOL_DOCUMENT_ROOT.'/custom/scolarite/class/salle.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'sallelist'; // To manage different context of search
...
$hookmanager->initHooks(array('sallelist')); // Note that conf->hooks_modules contains array
...
//$help_url="EN:Module_Salle|FR:Module_Salle_FR|ES:Módulo_Salle";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Salles"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/scolarite/salle/card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/scolarite/salle/card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendSalleRef";
$modelmail = "salle";
$objecttmp = new Salle($db);
