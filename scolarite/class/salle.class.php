...
/**
 * \file        class/salle.class.php
 * \ingroup     scolarite
 * \brief       This file is a CRUD class file for Salle (Create/Read/Update/Delete)
 */
...
class Salle extends CommonObject
{
	...
	//  */
	// public $table_element_line = 'scolarite_salleline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_salle';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Salleline';
	...
	//  */
	// protected $childtablesoncascade = array('scolarite_salledet');

	// /**
	//  * @var SalleLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SALLE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'salle/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'salle/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->scolarite->dir_output.'/salle/'.$oldref;
				$dirdest = $conf->scolarite->dir_output.'/salle/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->scolarite->dir_output.'/salle/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		// $label = talm_img_picto('', 'salle').' <u>'.$langs->trans("Salle").'</u>';
		...
		if (empty($this->showphoto_on_popup)) {
			if ($withpicto) {
				$result .= talm_img_object(($notooltip ? '' : $label), 'salle', ($notooltip ? (($withpicto != 2) ? 'class="paddingright"' : '') : 'class="'.(($withpicto != 2) ? 'paddingright ' : '').'classfortooltip"'), 0, 0, $notooltip ? 0 : 1);
			}
		} else {
			if ($withpicto) {
				require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
		...
		$hookmanager->initHooks(array('salledao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new SalleLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_salle = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->SCOLARITE_SALLE_ADDON)) {
			$conf->global->SCOLARITE_SALLE_ADDON = 'mod_salle_standard';
		}

		if (!empty($conf->global->SCOLARITE_SALLE_ADDON)) {
			$mybool = false;

			$file = $conf->global->SCOLARITE_SALLE_ADDON.".php";
			$classname = $conf->global->SCOLARITE_SALLE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_salle';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->SALLE_ADDON_PDF)) {
				$modele = $conf->global->SALLE_ADDON_PDF;
			}
		}
