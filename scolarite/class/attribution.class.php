...
/**
 * \file        class/attribution.class.php
 * \ingroup     scolarite
 ...
 */
class Attribution extends CommonObject
{
	/**
	...
	//  */
	// public $table_element_line = 'scolarite_attributionline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_attribution';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Attributionline';
	...
	//  */
	// protected $childtablesoncascade = array('scolarite_attributiondet');

	// /**
	//  * @var AttributionLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('ATTRIBUTION_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'attribution/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'attribution/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->scolarite->dir_output.'/attribution/'.$oldref;
				$dirdest = $conf->scolarite->dir_output.'/attribution/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->scolarite->dir_output.'/attribution/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Attribution").'</u>';
		...
		$url = dol_buildpath('/scolarite/attribution_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowAttribution");
		...
		$hookmanager->initHooks(array('attributiondao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new AttributionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_attribution = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->SCOLARITE_ATTRIBUTION_ADDON)) {
			$conf->global->SCOLARITE_ATTRIBUTION_ADDON = 'mod_attribution_standard';
		}

		if (!empty($conf->global->SCOLARITE_ATTRIBUTION_ADDON)) {
			$mybool = false;

			$file = $conf->global->SCOLARITE_ATTRIBUTION_ADDON.".php";
			$classname = $conf->global->SCOLARITE_ATTRIBUTION_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_attribution';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->ATTRIBUTION_ADDON_PDF)) {
				$modele = $conf->global->ATTRIBUTION_ADDON_PDF;
			}
		}
