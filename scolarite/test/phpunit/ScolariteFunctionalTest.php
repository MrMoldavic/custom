...
/**
 * \file    test/phpunit/ScolariteFunctionalTest.php
 * \ingroup scolarite
 ...
 *
 * @package Testscolarite
 */
class ScolariteFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modScolarite')->click();
		} else {
			// Disable the module
			$this->byHref('modScolarite')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/scolarite/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('scolarite/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/scolarite/admin/about.php');
		$this->authenticate();
		return $this->assertContains('scolarite/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('scolaritewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modScolarite_ScolariteTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modScolarite_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
