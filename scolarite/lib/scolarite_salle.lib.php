...
function sallePrepareHead($object)
{
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
		...
		$head[$h][0] = dol_buildpath('/scolarite/salle_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->scolarite->dir_output."/salle/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/scolarite/salle_document.php", 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/scolarite/salle_agenda.php", 1).'?id='.$object->id;
