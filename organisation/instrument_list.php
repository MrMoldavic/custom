...
/**
 *   	\file       instrument_list.php
 *		\ingroup    organisation
 *		\brief      List page for instrument
 */
...
require_once __DIR__.'/class/instrument.class.php';
...
$hookmanager->initHooks(array('instrumentlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->organisation->instrument->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Instrument|FR:Module_Instrument_FR|ES:Módulo_Instrument";
$help_url = '';
$title = $langs->trans("Instruments");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/organisation/instrument_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/organisation/instrument_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendInstrumentRef";
$modelmail = "instrument";
$objecttmp = new Instrument($db);
