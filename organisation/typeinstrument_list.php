...
/**
 *   	\file       typeinstrument_list.php
 *		\ingroup    organisation
 *		\brief      List page for typeinstrument
 */
...
require_once __DIR__.'/class/typeinstrument.class.php';
...
$hookmanager->initHooks(array('typeinstrumentlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->organisation->typeinstrument->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($conf->organisation->enabled)) accessforbidden('Module not enabled');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'TypeInstrument';
	$objectlabel = 'TypeInstrument';
...
//$help_url = "EN:Module_TypeInstrument|FR:Module_TypeInstrument_FR|ES:Módulo_TypeInstrument";
$help_url = '';
$title = $langs->trans("TypeInstruments");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/organisation/typeinstrument_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/organisation/typeinstrument_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendTypeInstrumentRef";
$modelmail = "typeinstrument";
$objecttmp = new TypeInstrument($db);
