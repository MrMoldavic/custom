...
/**
 *   	\file       liaisoninstrument_list.php
 *		\ingroup    organisation
 *		\brief      List page for liaisoninstrument
 */
...
require_once __DIR__.'/class/liaisoninstrument.class.php';
...
$hookmanager->initHooks(array('liaisoninstrumentlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('organisation', 'liaisoninstrument', 'write');
	$permissiontodelete = $user->hasRight('organisation', 'liaisoninstrument', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("organisation")) {
	accessforbidden('Module organisation not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'LiaisonInstrument';
	$objectlabel = 'LiaisonInstrument';
	$uploaddir = $conf->organisation->dir_output;
...
//$help_url = "EN:Module_LiaisonInstrument|FR:Module_LiaisonInstrument_FR|ES:Módulo_LiaisonInstrument";
$help_url = '';
$title = $langs->trans("LiaisonInstruments");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/organisation/liaisoninstrument_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/organisation/liaisoninstrument_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendLiaisonInstrumentRef";
$modelmail = "liaisoninstrument";
$objecttmp = new LiaisonInstrument($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_organisation', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
