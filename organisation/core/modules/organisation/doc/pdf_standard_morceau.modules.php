...
/**
 *	Class to manage PDF template standard_morceau
 */
class pdf_standard_morceau extends ModelePDFMorceau
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		/*
		if (!empty($conf->global->MAIN_GENERATE_MORCEAU_WITH_PICTURE))
		{
		...
		*/
		...
		if ($conf->organisation->dir_output.'/morceau') {
			$object->fetch_thirdparty();

			// Definition of $dir and $file
			if ($object->specimen) {
				$dir = $conf->organisation->dir_output.'/morceau';
				$file = $dir."/SPECIMEN.pdf";
			} else {
				$objectref = dol_sanitizeFileName($object->ref);
				$dir = $conf->organisation->dir_output.'/morceau/'.$objectref;
