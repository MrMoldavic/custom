...
/**
 * \file        class/artiste.class.php
 * \ingroup     organisation
 * \brief       This file is a CRUD class file for Artiste (Create/Read/Update/Delete)
 */
 ...
 * Class for Artiste
 */
class Artiste extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'artiste';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_artiste';
	...
	/**
	 * @var string String with name of icon for artiste. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'artiste@organisation' if picto is file 'img/object_artiste.png'.
	 */
	...
	//  */
	// public $table_element_line = 'organisation_artisteline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_artiste';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Artisteline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_artistedet');

	// /**
	//  * @var ArtisteLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->organisation->artiste->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('ARTISTE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'artiste/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'artiste/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/artiste/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/artiste/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/artiste/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Artiste").'</u>';
		...
		$url = dol_buildpath('/organisation/artiste_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('artistedao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new ArtisteLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_artiste = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_ARTISTE_ADDON)) {
			$conf->global->ORGANISATION_ARTISTE_ADDON = 'mod_artiste_standard';
		}

		if (!empty($conf->global->ORGANISATION_ARTISTE_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_ARTISTE_ADDON.".php";
			$classname = $conf->global->ORGANISATION_ARTISTE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_artiste';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->ARTISTE_ADDON_PDF)) {
				$modele = $conf->global->ARTISTE_ADDON_PDF;
			}
		}
...
/**
 * Class ArtisteLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ArtisteLine extends CommonObjectLine
{
	// To complete with content of an object ArtisteLine
	// We should have a field rowid, fk_artiste and position
