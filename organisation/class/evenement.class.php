...
/**
 * \file        class/evenement.class.php
 * \ingroup     organisation
 * \brief       This file is a CRUD class file for Evenement (Create/Read/Update/Delete)
 */
 ...
 * Class for Evenement
 */
class Evenement extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'evenement';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_evenement';
	...
	/**
	 * @var string String with name of icon for evenement. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'evenement@organisation' if picto is file 'img/object_evenement.png'.
	 */
	...
	//  */
	// public $table_element_line = 'organisation_evenementline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_evenement';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Evenementline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_evenementdet');

	// /**
	//  * @var EvenementLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->organisation->evenement->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->organisation->evenement->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->organisation->evenement->evenement_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('EVENEMENT_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'evenement/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'evenement/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/evenement/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/evenement/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/evenement/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'EVENEMENT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'EVENEMENT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'EVENEMENT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Evenement").'</u>';
		...
		$url = dol_buildpath('/organisation/evenement_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowEvenement");
		...
		$hookmanager->initHooks(array('evenementdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new EvenementLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_evenement = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_EVENEMENT_ADDON)) {
			$conf->global->ORGANISATION_EVENEMENT_ADDON = 'mod_evenement_standard';
		}

		if (!empty($conf->global->ORGANISATION_EVENEMENT_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_EVENEMENT_ADDON.".php";
			$classname = $conf->global->ORGANISATION_EVENEMENT_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_evenement';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->EVENEMENT_ADDON_PDF)) {
				$modele = $conf->global->EVENEMENT_ADDON_PDF;
			}
		}
...
/**
 * Class EvenementLine. You can also remove this and generate a CRUD class for lines objects.
 */
class EvenementLine extends CommonObjectLine
{
	// To complete with content of an object EvenementLine
	// We should have a field rowid, fk_evenement and position
