...
/**
 * \file        class/engagement.class.php
 ...
 */
class Engagement extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'engagement';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_engagement';
	...
	/**
	 * @var string String with name of icon for engagement. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'engagement@organisation' if picto is file 'img/object_engagement.png'.
	 */
	public $picto = 'fa-signature';
	...
	public $fields=array(
		...
		'fk_groupe' => array('type'=>'integer:Groupe:custom/organisation/class/groupe.class.php:1', 'label'=>'Groupe concerné', 'enabled'=>'1','css'=>'maxwidth300', 'position'=>20, 'notnull'=>1, 'visible'=>1, 'index'=>1, 'searchall'=>1, 'showoncombobox'=>'1', 'validate'=>'1'),
	...
	//  */
	// public $table_element_line = 'organisation_engagementline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_engagement';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Engagementline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_engagementdet');

	// /**
	//  * @var EngagementLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('ENGAGEMENT_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'engagement/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'engagement/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/engagement/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/engagement/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/engagement/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Engagement").'</u>';
		...
		$url = dol_buildpath('/organisation/engagement_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowEngagement");
		...
		$hookmanager->initHooks(array('engagementdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new EngagementLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_engagement = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_ENGAGEMENT_ADDON)) {
			$conf->global->ORGANISATION_ENGAGEMENT_ADDON = 'mod_engagement_standard';
		}

		if (!empty($conf->global->ORGANISATION_ENGAGEMENT_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_ENGAGEMENT_ADDON.".php";
			$classname = $conf->global->ORGANISATION_ENGAGEMENT_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_engagement';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->ENGAGEMENT_ADDON_PDF)) {
				$modele = $conf->global->ENGAGEMENT_ADDON_PDF;
			}
		}
...
/**
 * Class EngagementLine. You can also remove this and generate a CRUD class for lines objects.
 */
class EngagementLine extends CommonObjectLine
{
	// To complete with content of an object EngagementLine
	// We should have a field rowid, fk_engagement and position
