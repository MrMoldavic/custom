 ...
 * Class for Morceau
 */
class Morceau extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'morceau';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_morceau';
	...
	/**
	 * @var string String with name of icon for morceau. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'morceau@organisation' if picto is file 'img/object_morceau.png'.
	 */
	...
	//  */
	// public $table_element_line = 'organisation_morceauline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_morceau';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Morceauline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_morceaudet');

	// /**
	//  * @var MorceauLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('MORCEAU_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'morceau/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'morceau/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/morceau/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/morceau/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/morceau/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Morceau").'</u>';
		...
		$url = dol_buildpath('/organisation/morceau_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('morceaudao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new MorceauLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_morceau = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_MORCEAU_ADDON)) {
			$conf->global->ORGANISATION_MORCEAU_ADDON = 'mod_morceau_standard';
		}

		if (!empty($conf->global->ORGANISATION_MORCEAU_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_MORCEAU_ADDON.".php";
			$classname = $conf->global->ORGANISATION_MORCEAU_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_morceau';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->MORCEAU_ADDON_PDF)) {
				$modele = $conf->global->MORCEAU_ADDON_PDF;
			}
		}
...
/**
 * Class MorceauLine. You can also remove this and generate a CRUD class for lines objects.
 */
class MorceauLine extends CommonObjectLine
{
	// To complete with content of an object MorceauLine
	// We should have a field rowid, fk_morceau and position
