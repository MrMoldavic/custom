...
/**
 * \file        class/proposition.class.php
 * \ingroup     organisation
 * \brief       This file is a CRUD class file for Proposition (Create/Read/Update/Delete)
 */
 ...
 */
class Proposition extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'proposition';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_proposition';
	...
	/**
	 * @var string String with name of icon for proposition. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'proposition@organisation' if picto is file 'img/object_proposition.png'.
	 */
	...
	//  */
	// public $table_element_line = 'organisation_propositionline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_proposition';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Propositionline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_propositiondet');

	// /**
	//  * @var PropositionLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->organisation->proposition->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->organisation->proposition->proposition_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('PROPOSITION_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'proposition/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'proposition/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/proposition/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/proposition/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/proposition/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Proposition").'</u>';
		...
		$url = dol_buildpath('/organisation/proposition_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowProposition");
		...
		$hookmanager->initHooks(array('propositiondao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new PropositionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_proposition = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_PROPOSITION_ADDON)) {
			$conf->global->ORGANISATION_PROPOSITION_ADDON = 'mod_proposition_standard';
		}

		if (!empty($conf->global->ORGANISATION_PROPOSITION_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_PROPOSITION_ADDON.".php";
			$classname = $conf->global->ORGANISATION_PROPOSITION_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_proposition';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->PROPOSITION_ADDON_PDF)) {
				$modele = $conf->global->PROPOSITION_ADDON_PDF;
			}
		}
...
/**
 * Class PropositionLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PropositionLine extends CommonObjectLine
{
	// To complete with content of an object PropositionLine
	// We should have a field rowid, fk_proposition and position
