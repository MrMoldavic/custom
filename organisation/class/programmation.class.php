...
/**
 * \file        class/programmation.class.php
...
/**
 * Class for Programmation
 */
class Programmation extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'programmation';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'organisation_programmation';
	...
	/**
	 * @var string String with name of icon for programmation. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'programmation@organisation' if picto is file 'img/object_programmation.png'.
	 */
	...
	//  */
	// public $table_element_line = 'organisation_programmationline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_programmation';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Programmationline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_programmationdet');

	// /**
	//  * @var ProgrammationLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->organisation->programmation->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->organisation->programmation->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->organisation->programmation->programmation_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('PROGRAMMATION_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'programmation/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'programmation/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->organisation->dir_output.'/programmation/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/programmation/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/programmation/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'PROGRAMMATION_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'PROGRAMMATION_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'PROGRAMMATION_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Programmation").'</u>';
		...
		$url = dol_buildpath('/organisation/programmation_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowProgrammation");
	...
	*/
	...
	public function getLinesArray()
	{
		...
		$objectline = new ProgrammationLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_programmation = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_PROGRAMMATION_ADDON)) {
			$conf->global->ORGANISATION_PROGRAMMATION_ADDON = 'mod_programmation_standard';
		}

		if (!empty($conf->global->ORGANISATION_PROGRAMMATION_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_PROGRAMMATION_ADDON.".php";
			$classname = $conf->global->ORGANISATION_PROGRAMMATION_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_programmation';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->PROGRAMMATION_ADDON_PDF)) {
				$modele = $conf->global->PROGRAMMATION_ADDON_PDF;
			}
		}
...
/**
 * Class ProgrammationLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ProgrammationLine extends CommonObjectLine
{
	// To complete with content of an object ProgrammationLine
	// We should have a field rowid, fk_programmation and position
