...
class Groupe extends CommonObject
{
	 ...
	 */
	public $table_element = 'organisation_groupe';
	...
	/**
	 * @var string String with name of icon for groupe. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'groupe@organisation' if picto is file 'img/object_groupe.png'.
	 */
	public $picto = 'fa-music';
	...
	//  */
	// public $table_element_line = 'organisation_groupeline';
	...
	//  */
	// protected $childtablesoncascade = array('organisation_groupedet');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('GROUPE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->organisation->dir_output.'/groupe/'.$oldref;
				$dirdest = $conf->organisation->dir_output.'/groupe/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->organisation->dir_output.'/groupe/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'GROUPE_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'GROUPE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'GROUPE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Groupe").'</u>';
		...
		$url = dol_buildpath('/organisation/groupe_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('groupedao'));
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('Groupe en attente');
			$this->labelStatus[self::STATUS_VALIDATED] = $langs->transnoentitiesnoconv('Groupe Actif');
			$this->labelStatus[self::STATUS_CANCELED] = $langs->transnoentitiesnoconv('Groupe à la retraite');
			$this->labelStatusShort[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('Groupe en attente');
			$this->labelStatusShort[self::STATUS_VALIDATED] = $langs->transnoentitiesnoconv('Groupe Actif');
			$this->labelStatusShort[self::STATUS_CANCELED] = $langs->transnoentitiesnoconv('Groupe à la retraite');
		}
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->ORGANISATION_GROUPE_ADDON)) {
			$conf->global->ORGANISATION_GROUPE_ADDON = 'mod_groupe_standard';
		}

		if (!empty($conf->global->ORGANISATION_GROUPE_ADDON)) {
			$mybool = false;

			$file = $conf->global->ORGANISATION_GROUPE_ADDON.".php";
			$classname = $conf->global->ORGANISATION_GROUPE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (!empty($conf->global->GROUPE_ADDON_PDF)) {
				$modele = $conf->global->GROUPE_ADDON_PDF;
			}
		}
