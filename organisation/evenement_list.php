...
/**
 *   	\file       evenement_list.php
 *		\ingroup    organisation
 *		\brief      List page for evenement
 */
...
require_once __DIR__.'/class/evenement.class.php';
...
$hookmanager->initHooks(array('evenementlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->organisation->evenement->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Evenement';
	$objectlabel = 'Evenement';
...
//$help_url = "EN:Module_Evenement|FR:Module_Evenement_FR|ES:Módulo_Evenement";
$help_url = '';
$title = $langs->trans("Evenements");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/organisation/evenement_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/organisation/evenement_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEvenementRef";
$modelmail = "evenement";
$objecttmp = new Evenement($db);
	...
	if ($mode == 'kanban') {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				elseif ($key == 'nom_evenement') {
					print $object->getNomUrl();
				}
				elseif ($key == 'fk_etat_evenement') {
					print '<span class="badge  badge-status'.$object->fk_etat_evenement.' badge-status" style="color:white;">'.$object->showOutputField($val, $key, $object->$key, '').'</span>';
				}
				elseif ($key == 'rowid') {
