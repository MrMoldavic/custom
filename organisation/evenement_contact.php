...
/**
 *  \file       evenement_contact.php
 *  \ingroup    organisation
 *  \brief      Tab for contacts linked to Evenement
 */
...
$hookmanager->initHooks(array('evenementcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->organisation->evenement->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Evenement')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Evenement"), -1, $object->picto);
