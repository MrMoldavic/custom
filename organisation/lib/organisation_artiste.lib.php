...
function artistePrepareHead($object)
{
	...
	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/organisation/artiste_contact.php", 1).'?id='.$object->id;
	...
	if ($showtabofpagenote) {
		if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
			...
			$head[$h][0] = dol_buildpath('/organisation/artiste_note.php', 1).'?id='.$object->id;
	...
	if ($showtabofpagedocument) {
		...
		$upload_dir = $conf->organisation->dir_output."/artiste/".dol_sanitizeFileName($object->ref);
		$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
		$nbLinks = Link::count($db, $object->element, $object->id);
		$head[$h][0] = dol_buildpath("/organisation/artiste_document.php", 1).'?id='.$object->id;
	...
	if ($showtabofpageagenda) {
		$head[$h][0] = dol_buildpath("/organisation/artiste_agenda.php", 1).'?id='.$object->id;
