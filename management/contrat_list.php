...
/**
 *   	\file       contrat_list.php
 *		\ingroup    management
 *		\brief      List page for contrat
 */
...
require_once __DIR__.'/class/contrat.class.php';
...
$hookmanager->initHooks(array('contratlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->management->contrat->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Contrat|FR:Module_Contrat_FR|ES:Módulo_Contrat";
$help_url = '';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/management/contrat_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/management/contrat_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
	...
	if ($mode == 'kanban') {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				elseif ($key == 'alternance') {
					print ($object->alternance == 1 ? 'OUI' : 'NON');
				}
				elseif ($key == 'rowid') {
