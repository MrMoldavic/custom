...
/**
 *  \file       agent_contact.php
 *  \ingroup    management
 *  \brief      Tab for contacts linked to Agent
 */
...
$hookmanager->initHooks(array('agentcontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->management->agent->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Agent')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Agent"), -1, $object->picto);
