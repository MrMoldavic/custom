...
/**
 *   	\file       appetence_list.php
 *		\ingroup    management
 *		\brief      List page for appetence
 */
...
require_once __DIR__.'/class/appetence.class.php';
...
$hookmanager->initHooks(array('appetencelist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('management', 'appetence', 'write');
	$permissiontodelete = $user->hasRight('management', 'appetence', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("management")) {
	accessforbidden('Module management not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Appetence';
	$objectlabel = 'Appetence';
...
//$help_url = "EN:Module_Appetence|FR:Module_Appetence_FR|ES:Módulo_Appetence";
$help_url = '';
$title = $langs->trans("Appetences");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/management/appetence_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/management/appetence_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAppetenceRef";
$modelmail = "appetence";
$objecttmp = new Appetence($db);
