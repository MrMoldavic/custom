...
/**
 * \file        class/agent.class.php
 * \ingroup     management
 * \brief       This file is a CRUD class file for Agent (Create/Read/Update/Delete)
 */
 ...
 * Class for Agent
 */
class Agent extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'agent';

	/**
	 * @var string ID to identify managed object.
	 */
	public $element_appel = 'user';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'management_agent';
	...
	/**
	 * @var string String with name of icon for agent. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'agent@management' if picto is file 'img/object_agent.png'.
	 */
	...
	public $fields=array(
		...
		'tms' => array('type'=>'date', 'label'=>'Lieu de naissance', 'enabled'=>'1', 'position'=>501, 'notnull'=>0, 'visible'=>-2,),
	...
	//  */
	// public $table_element_line = 'management_agentline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_agent';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Agentline';
	...
	//  */
	// protected $childtablesoncascade = array('management_agentdet');

	// /**
	//  * @var AgentLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->management->agent->read) {
	...
	{
		$agent = "SELECT s.fk_user FROM ".MAIN_DB_PREFIX."management_agent as s WHERE s.fk_user =".$this->fk_user;
		...
		else
		{
			...
			$resqlUser= $this->db->query($userCible);
			$objectUser =  $this->db->fetch_object($resqlUser);

			if((strtolower($objectUser->lastname) != strtolower($this->nom)) || (strtolower($objectUser->firstname) != strtolower($this->prenom)))
			{
				setEventMessage('Les informations ne correspondent pas avec l\'utilisateur cible.','errors');
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->management->agent->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->management->agent->agent_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('AGENT_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'agent/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'agent/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->management->dir_output.'/agent/'.$oldref;
				$dirdest = $conf->management->dir_output.'/agent/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->management->dir_output.'/agent/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'AGENT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'AGENT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'AGENT_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Agent").'</u>';
		...
		$url = dol_buildpath('/management/agent_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('agentdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new AgentLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_agent = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->MANAGEMENT_AGENT_ADDON)) {
			$conf->global->MANAGEMENT_AGENT_ADDON = 'mod_agent_standard';
		}

		if (!empty($conf->global->MANAGEMENT_AGENT_ADDON)) {
			$mybool = false;

			$file = $conf->global->MANAGEMENT_AGENT_ADDON.".php";
			$classname = $conf->global->MANAGEMENT_AGENT_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_agent';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->AGENT_ADDON_PDF)) {
				$modele = $conf->global->AGENT_ADDON_PDF;
			}
		}
...
/**
 * Class AgentLine. You can also remove this and generate a CRUD class for lines objects.
 */
class AgentLine extends CommonObjectLine
{
	// To complete with content of an object AgentLine
	// We should have a field rowid, fk_agent and position
