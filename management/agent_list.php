...
require_once __DIR__.'/class/agent.class.php';
...
$hookmanager->initHooks(array('agentlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->management->agent->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Agent|FR:Module_Agent_FR|ES:Módulo_Agent";
$help_url = '';
$title = $langs->trans("Agents");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/management/agent_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/management/agent_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAgentRef";
$modelmail = "agent";
$objecttmp = new Agent($db);
