...
function typestatutPrepareHead($object)
{
	...
	if ($showtabofpagecontact) {
		$head[$h][0] = dol_buildpath("/management/typestatut_contact.php", 1).'?id='.$object->id;
	...
	if ($showtabofpagenote) {
		if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
			...
			$head[$h][0] = dol_buildpath('/management/typestatut_note.php', 1).'?id='.$object->id;
	...
	if ($showtabofpagedocument) {
		...
		$upload_dir = $conf->management->dir_output."/typestatut/".dol_sanitizeFileName($object->ref);
		$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
		$nbLinks = Link::count($db, $object->element, $object->id);
		$head[$h][0] = dol_buildpath("/management/typestatut_document.php", 1).'?id='.$object->id;
	...
	if ($showtabofpageagenda) {
		$head[$h][0] = dol_buildpath("/management/typestatut_agenda.php", 1).'?id='.$object->id;
