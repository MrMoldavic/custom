...
/**
 *   	\file       statut_list.php
 *		\ingroup    management
 *		\brief      List page for statut
 */
...
require_once __DIR__.'/class/statut.class.php';
...
$hookmanager->initHooks(array('statutlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->management->statut->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Statut|FR:Module_Statut_FR|ES:Módulo_Statut";
$help_url = '';
$title = $langs->trans("Statuts");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/management/statut_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/management/statut_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendStatutRef";
$modelmail = "statut";
$objecttmp = new Statut($db);
