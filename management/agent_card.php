 ...
 *   	\file       agent_card.php
 *		\ingroup    management
 *		\brief      Page to create/edit/view agent
 */
...
$hookmanager->initHooks(array('agentcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/management/agent_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/management/agent_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}

	$triggermodname = 'MANAGEMENT_AGENT_MODIFY'; // Name of trigger action code to execute when we modify record
	...
	$triggersendname = 'MANAGEMENT_AGENT_SENTBYMAIL';
	$autocopy = 'MAIN_MAIL_AUTOCOPY_AGENT_TO';
	$trackid = 'agent'.$object->id;
...
if ($action == 'create') {
	...
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Agent")), '', 'object_'.$object->picto);
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Agent"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	print dol_get_fiche_head($head, 'card', $langs->trans("Agent"), -1, $object->picto);

	$formconfirm = '';

	// Confirmation to delete
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteAgent'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// $status = "SELECT nom, rowid FROM ".MAIN_DB_PREFIX."management_typestatut WHERE rowid=(SELECT s.fk_type_statut FROM ".MAIN_DB_PREFIX."management_statut WHERE fk_agent=".$object->id.")";
	$status = "SELECT s.fk_type_statut FROM ".MAIN_DB_PREFIX."management_statut as s WHERE s.fk_agent =".$object->id;
	$resqlStatus= $db->query($status);
	...
	$morehtmlref.= '&nbsp;&nbsp;'.$object->prenom.'<br>';

	if($resqlStatus->num_rows == 0)
	{
		$morehtmlref.= '<span class="badge  badge-status8 badge-status">Aucun Statut</span>    ';
	}
	else
	{
	...
	print '</table>';
	print '<h3>Liste des appentences de l\'agent:</h3>';

	$appetence = "SELECT fk_type_poste, fk_type_appetence,rowid FROM ".MAIN_DB_PREFIX."management_appetence WHERE fk_agent =".$object->id;
	$resqlAppetence= $db->query($appetence);

	if($resqlAppetence->num_rows == 0)
	{
		print '<span class="badge badge-status8 badge-status">Aucun Statut</span>';
	}
	else
	{
