...
/**
 * \file    test/phpunit/EntretienFunctionalTest.php
 * \ingroup entretien
 ...
 */
class EntretienFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modEntretien')->click();
		} else {
			// Disable the module
			$this->byHref('modEntretien')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/entretien/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('entretien/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/entretien/admin/about.php');
		$this->authenticate();
		return $this->assertContains('entretien/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('entretienwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modEntretien_EntretienTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modEntretien_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
