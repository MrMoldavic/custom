...
elseif ($action == 'add')
{
	...
	if (!$error) {
		...
		$recufiscal->fk_donateur = $donateur;
...
elseif ($action == 'confirm_edit' && $confirm == 'yes')
{
...
if ($action == 'create' && $usercancreate)
{
    ...
    print $form->selectarray('donateur', getDonateurArray(), $donateur, 1, 0, 0, 'style="min-width:200px;"', 0, 0, 0, '', '', 1);
...
elseif ($recufiscal->id > 0) {
    ...
    print dol_print_date($recufiscal->datec, '%e/%m/%Y');
	 ...
	 * Generated documents
	 */
	$filename = dol_sanitizeFileName($recufiscal->id);
	$filedir = $conf->materiel->dir_output."/".dol_sanitizeFileName($recufiscal->id);
	...
	print $materiel->showdocuments('recu_fiscal', $filename, $filedir, $urlsource, $genallowed, $delallowed, $model);
...

    {
        ...
        if (empty($reshook)) {
            ...
            if ($usercancreate && $recufiscal->fk_statut == RecuFiscal::STATUS_VALIDATED) {
                ...
                print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=edit&amp;id='.$recufiscal->id.'">'.$langs->trans("Modify").'</a>';
