...
/**
 *   	\file       parents_list.php
 *		\ingroup    viescolaire
 *		\brief      List page for parents
 */
...
require_once __DIR__.'/class/parents.class.php';
...
$hookmanager->initHooks(array('parentslist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('viescolaire', 'parents', 'write');
	$permissiontodelete = $user->hasRight('viescolaire', 'parents', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/viescolaire/parents_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/viescolaire/parents_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendParentsRef";
$modelmail = "parents";
$objecttmp = new Parents($db);
