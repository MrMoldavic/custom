...
/**
 *   	\file       famille_list.php
 *		\ingroup    viescolaire
 *		\brief      List page for famille
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'famillelist'; // To manage different context of search
...
$hookmanager->initHooks(array('famillelist')); // Note that conf->hooks_modules contains array
...
//$help_url="EN:Module_Famille|FR:Module_Famille_FR|ES:Módulo_Famille";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("Familles"));
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/viescolaire/famille_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/viescolaire/famille_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendFamilleRef";
$modelmail = "famille";
$objecttmp = new Famille($db);
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				if ($key == 'status') {
					...
					{
						if(!$value->mail || !$value->phone) {
							$error++;
						}
					}
					if(count($parentsFromFamily) === 0) {
						print ' <span style="font-size: 1em" class="badge badge-danger">Responsables manquants '.img_picto('', 'fa-warning', 'class="valignmiddle"').'</span><br>';
					}
					elseif($error > 0) {
						print ' <span style="font-size: 1em"  class="badge badge-danger">Mail ou Téléphone manquant '.img_picto('', 'fa-warning', 'class="valignmiddle"').'</span>';
					}
					print '</a>';
				} elseif ($key == 'enfants') {
					...
					{
						print '<li>'.$enfant->getNomUrl(1).'</li>';
					}
					print '</ul>';
