...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'Materiel', $langs->trans("Eleve"), -1, $object->picto);
	...
	$sqlExploitation = "SELECT rowid,cote,fk_type_exploitation FROM ".MAIN_DB_PREFIX."exploitation WHERE fk_exploitant = ".$object->id." AND fk_type_exploitation= 1 AND active = 1";
	$resqlExploitation = $db->query($sqlExploitation);


	if(!$resqlExploitation)
	{
	...
	else
	{
		...
		foreach($resqlExploitation as $val)
		{

			$sqlInstrumentsPretes = "SELECT fk_materiel FROM ".MAIN_DB_PREFIX."kit_content WHERE fk_kit =(SELECT fk_kit FROM ".MAIN_DB_PREFIX."exploitation_content WHERE fk_exploitation =".$val['rowid'].")";
			$resqlInstruments = $db->query($sqlInstrumentsPretes);

			$sqlTypeExploit = "SELECT indicatif FROM ".MAIN_DB_PREFIX."c_type_exploitation WHERE rowid=".$val['fk_type_exploitation'];
			...
			foreach($resqlInstruments as $value)
			{
				...
				{
					$cote = "00".$objInstrument->cote;
				}
				elseif(strlen($objInstrument->cote) == 2)
				{
					$cote = "0".$objInstrument->cote;
				}
				else
				{
				...
				print '<td><span class="badge  badge-status'.$objEtat->badge_status_code.' badge-status">'.$objEtat->etat.'</td>';
