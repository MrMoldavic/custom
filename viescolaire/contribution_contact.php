...
/**
 *  \file       contribution_contact.php
 *  \ingroup    viescolaire
 *  \brief      Tab for contacts linked to Contribution
 */
...
$hookmanager->initHooks(array('contributioncontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->viescolaire->contribution->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('Contribution')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("Contribution"), -1, $object->picto);
