...
/**
 * \file        class/eleve.class.php
 * \ingroup     viescolaire
 * \brief       This file is a CRUD class file for Eleve (Create/Read/Update/Delete)
 */
 ...
 * Class for Eleve
 */
class Eleve extends CommonObject
{
	/**
	...
	//  */
	// public $table_element_line = 'viescolaire_eleveline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_eleve';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Eleveline';
	...
	// /**
	//  * @var EleveLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('ELEVE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filename = CONCAT('" . $this->db->escape($this->newref) . "', SUBSTR(filename, " . (strlen($this->ref) + 1) . ")), filepath = 'eleve/" . $this->db->escape($this->newref) . "'";
				$sql .= " WHERE filename LIKE '" . $this->db->escape($this->ref) . "%' AND filepath = 'eleve/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->viescolaire->dir_output . '/eleve/' . $oldref;
				$dirdest = $conf->viescolaire->dir_output . '/eleve/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->viescolaire->dir_output . '/eleve/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'ELEVE_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'ELEVE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'ELEVE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		// $label = img_picto('', $this->picto).' <u>'.$langs->trans("Eleve").'</u>';
		...
		$url = dol_buildpath('/viescolaire/eleve_card.php', 1) . '?id=' . $this->id;
		...
		$hookmanager->initHooks(array('elevedao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new EleveLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql' => 'fk_eleve = ' . ((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->VIESCOLAIRE_ELEVE_ADDON)) {
			$conf->global->VIESCOLAIRE_ELEVE_ADDON = 'mod_eleve_standard';
		}

		if (!empty($conf->global->VIESCOLAIRE_ELEVE_ADDON)) {
			$mybool = false;

			$file = $conf->global->VIESCOLAIRE_ELEVE_ADDON . ".php";
			$classname = $conf->global->VIESCOLAIRE_ELEVE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_eleve';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->ELEVE_ADDON_PDF)) {
				$modele = $conf->global->ELEVE_ADDON_PDF;
			}
		}
