...
/**
 * \file        class/inscription.class.php
 * \ingroup     viescolaire
 * \brief       This file is a CRUD class file for Inscription (Create/Read/Update/Delete)
 */
 ...
 */
class Inscription extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'inscription';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'viescolaire_inscription';
	...
	public $fields=array(
		...
		'tms' => array('type'=>'timestamp', 'label'=>'DateModification', 'enabled'=>'1', 'position'=>35, 'notnull'=>1, 'visible'=>0,),
		'fk_user_creat' => array('type'=>'integer:User:user/class/user.class.php', 'label'=>'UserAuthor', 'enabled'=>'1', 'position'=>40, 'notnull'=>1, 'visible'=>-2, 'css'=>'maxwidth500 widthcentpercentminusxx', 'csslist'=>'tdoverflowmax150',),
		'fk_user_modif' => array('type'=>'integer:User:user/class/user.class.php', 'label'=>'UserModif', 'enabled'=>'1', 'position'=>45, 'notnull'=>-1, 'visible'=>-2, 'css'=>'maxwidth500 widthcentpercentminusxx', 'csslist'=>'tdoverflowmax150',),
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('INSCRIPTION_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'inscription/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'inscription/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->viescolaire->dir_output.'/inscription/'.$oldref;
				$dirdest = $conf->viescolaire->dir_output.'/inscription/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->viescolaire->dir_output.'/inscription/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Inscription").'</u>';
		...
		$url = dol_buildpath('/viescolaire/inscription_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans("ShowInscription");
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('En cours d\'inscription');
			$this->labelStatus[self::STATUS_VALIDATED] = $langs->transnoentitiesnoconv('Inscription valide');
			$this->labelStatus[self::STATUS_REMOTIVER] = $langs->transnoentitiesnoconv('Ancien à remotiver');
			$this->labelStatus[self::STATUS_NON_REINSCRIT] = $langs->transnoentitiesnoconv('Non réinscrit');
			$this->labelStatus[self::STATUS_CANCELED] = $langs->transnoentitiesnoconv('Abandon');
			$this->labelStatusShort[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('En cours d\'inscription');
			$this->labelStatusShort[self::STATUS_VALIDATED] = $langs->transnoentitiesnoconv('Inscription valide');
			$this->labelStatusShort[self::STATUS_REMOTIVER] = $langs->transnoentitiesnoconv('Ancien à remotiver');
			$this->labelStatusShort[self::STATUS_NON_REINSCRIT] = $langs->transnoentitiesnoconv('Non réinscrit');
			$this->labelStatusShort[self::STATUS_CANCELED] = $langs->transnoentitiesnoconv('Abandon');
		}
	...
	public function getLinesArray()
	{
		...
		$objectline = new InscriptionLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_inscription = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->VIESCOLAIRE_INSCRIPTION_ADDON)) {
			$conf->global->VIESCOLAIRE_INSCRIPTION_ADDON = 'mod_inscription_standard';
		}

		if (!empty($conf->global->VIESCOLAIRE_INSCRIPTION_ADDON)) {
			$mybool = false;

			$file = $conf->global->VIESCOLAIRE_INSCRIPTION_ADDON.".php";
			$classname = $conf->global->VIESCOLAIRE_INSCRIPTION_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_inscription';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->INSCRIPTION_ADDON_PDF)) {
				$modele = $conf->global->INSCRIPTION_ADDON_PDF;
			}
		}
