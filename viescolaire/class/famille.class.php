...
/**
 * \file        class/famille.class.php
 * \ingroup     viescolaire
 * \brief       This file is a CRUD class file for Famille (Create/Read/Update/Delete)
 */
 ...
 * Class for Famille
 */
class Famille extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'famille';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'famille';
	...
	//  */
	// public $table_element_line = 'viescolaire_familleline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_famille';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Familleline';
	...
	//  */
	// protected $childtablesoncascade = array('viescolaire_familledet');

	// /**
	//  * @var FamilleLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('FAMILLE_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'famille/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'famille/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->viescolaire->dir_output.'/famille/'.$oldref;
				$dirdest = $conf->viescolaire->dir_output.'/famille/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->viescolaire->dir_output.'/famille/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		// $label = img_picto('', $this->picto).' <u>'.$langs->trans("Famille").'</u>';
		...
		$url = dol_buildpath('/viescolaire/famille_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('familledao'));
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('Non budgétisé');
			$this->labelStatus[self::STATUS_BUDGETISE] = $langs->transnoentitiesnoconv('Budgétisé');
			$this->labelStatus[self::STATUS_ENCOURS] = $langs->transnoentitiesnoconv('En cours de paiement');
			...
			$this->labelStatusShort[self::STATUS_ENCOURS] = $langs->transnoentitiesnoconv('En cours de paiement');
	...
	public function getLinesArray()
	{
		...
		$objectline = new FamilleLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_famille = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->VIESCOLAIRE_FAMILLE_ADDON)) {
			$conf->global->VIESCOLAIRE_FAMILLE_ADDON = 'mod_famille_standard';
		}

		if (!empty($conf->global->VIESCOLAIRE_FAMILLE_ADDON)) {
			$mybool = false;

			$file = $conf->global->VIESCOLAIRE_FAMILLE_ADDON.".php";
			$classname = $conf->global->VIESCOLAIRE_FAMILLE_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_famille';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->FAMILLE_ADDON_PDF)) {
				$modele = $conf->global->FAMILLE_ADDON_PDF;
			}
		}
