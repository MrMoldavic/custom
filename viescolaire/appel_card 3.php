...
if ($action == 'confirmAppel') {
     ...
     if (!$check) {
          ...
          foreach ($sqlEleves as $val) {
               var_dump(date('Y-m-d H:i:s', mktime(0, 0, 0, GETPOST('month', 'alpha'), GETPOST('daymonth', 'alpha'), date(GETPOST('year', 'alpha')))));
     ...
     setEventMessage("Appel validé pour ce créneau!");
...
if (($action == 'create' or $action == 'modifAppel') && GETPOST('etablissementid', 'int'))  // Type de kit choisi -> création d'un nouveau kit
{
     ...
     if(GETPOST('daymonth', 'alpha') || GETPOST('appelday', 'alpha'))
     {
          if(GETPOST('appelday', 'alpha') == "")
          {
               $day = GETPOST('day', 'alpha');
          }
          else
          {
               $day = GETPOST('appelday', 'alpha');
          }
     }
     else
     {
     ...
     if(GETPOST('month', 'alpha') || GETPOST('appelmonth', 'alpha'))
     {
          if(GETPOST('appelmonth', 'alpha') == "")
          {
               $month = GETPOST('month', 'alpha');
          }
          else
          {
               $month = GETPOST('appelmonth', 'alpha');
          }
     }
     else
     {
     ...
     if(GETPOST('year', 'alpha') || GETPOST('appelyear', 'alpha'))
     {
          if(GETPOST('appelyear', 'alpha') == "")
          {
               $year = GETPOST('year', 'alpha');
          }
          else
          {
               $year = GETPOST('appelyear', 'alpha');
               $dateComplete =  date("Y-m-d H:i:s", mktime(0, 0, 0, GETPOST('appelmonth', 'alpha'), GETPOST('appelday', 'alpha'), GETPOST('appelyear', 'alpha')));
               $JourSemaine = strftime('%u', strtotime($dateComplete));
               $heureActuelle = "23";
          }
     ...
     // Formulaire pour la sélection de la date d'appel
     print '<form action="' . $_SERVER["PHP_SELF"] . '" method="POST">';
     print '<input type="hidden" name="action" value="create">';
     print '<input type="hidden" name="etablissementid" value="'.GETPOST('etablissementid', 'int').'">';
     print '<table class="border centpercent">';
     print '<tr><th class="fieldrequired">Selectionnez une date: </th><th>';
     if(isset($dateComplete))
     {
          print $form->selectDate($dateComplete, 'appel');
     }
     else
     {
          print $form->selectDate('', 'appel');
     }
     print '<input type="submit" class="button" style="background-color:cadetblue">';
     print '</th>';
     print '</tr>';
     print "</table>";
     print '</form>';
