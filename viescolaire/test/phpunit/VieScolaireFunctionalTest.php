...
/**
 * \file    test/phpunit/VieScolaireFunctionalTest.php
 * \ingroup viescolaire
...
class VieScolaireFunctionalTest extends TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modVieScolaire')->click();
		} else {
			// Disable the module
			$this->byHref('modVieScolaire')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/viescolaire/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('viescolaire/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/viescolaire/admin/about.php');
		$this->authenticate();
		return $this->assertContains('viescolaire/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('viescolairewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modVieScolaire_VieScolaireTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modVieScolaire_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
