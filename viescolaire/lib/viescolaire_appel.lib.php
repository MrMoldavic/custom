...
function appelPrepareHead($object)
{
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
		...
		$head[$h][0] = dol_buildpath('/viescolaire/appel_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->viescolaire->dir_output."/appel/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/viescolaire/appel_document.php", 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/viescolaire/appel_agenda.php", 1).'?id='.$object->id;
