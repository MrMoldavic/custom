...
/**
 *   	\file       contribution_list.php
 *		\ingroup    viescolaire
 *		\brief      List page for contribution
 */
...
$hookmanager->initHooks(array('contributionlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('viescolaire', 'contribution', 'read');
	$permissiontoadd = $user->hasRight('viescolaire', 'contribution', 'write');
	$permissiontodelete = $user->hasRight('viescolaire', 'contribution', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
//$help_url = "EN:Module_Contribution|FR:Module_Contribution_FR|ES:Módulo_Contribution";
$help_url = '';
$title = $langs->trans("Contributions");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/viescolaire/contribution_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/viescolaire/contribution_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendContributionRef";
$modelmail = "contribution";
$objecttmp = new Contribution($db);
