...
/**
 * \file    test/phpunit/RecuFiscalFunctionalTest.php
 * \ingroup recufiscal
...
/**
 * Class RecuFiscalFunctionalTest
 *
 ...
 *
 * @package Testrecufiscal
 */
class RecuFiscalFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modRecuFiscal')->click();
		} else {
			// Disable the module
			$this->byHref('modRecuFiscal')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/recufiscal/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('recufiscal/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/recufiscal/admin/about.php');
		$this->authenticate();
		return $this->assertContains('recufiscal/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('recufiscalwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modRecuFiscal_RecuFiscalTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modRecuFiscal_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
