...
/**
 * \file    recufiscal/admin/setup.php
 * \ingroup recufiscal
 * \brief   RecuFiscal setup page.
 */
...
require_once '../lib/recufiscal.lib.php';

require_once DOL_DOCUMENT_ROOT."/custom/materiel/class/recufiscal.class.php";

// Translations
$langs->loadLangs(array("admin", "recufiscal@recufiscal"));
...
$arrayofparameters = array(
	'RECUFISCAL_MYPARAM1'=>array('css'=>'minwidth200', 'enabled'=>1),
	'RECUFISCAL_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	$tmpobject->initAsSpecimenCommon();
	...
	{
		$file = dol_buildpath($reldir."custom/recufiscal/core/modules/doc/pdf_".$modele.".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'RECUFISCAL_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'RECUFISCAL_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "RecuFiscalSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_recufiscal@recufiscal');

// Configuration header
$head = recufiscalAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "recufiscal@recufiscal");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("RecuFiscalSetupPage").'</span><br><br>';
...
$myTmpObjects = array();
$myTmpObjects['RecuFiscal'] = array('includerefgeneration'=>0, 'includedocgeneration'=>1);
...
foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'RECUFISCAL_'.strtoupper($myTmpObjectKey).'_ADDON';
	...
	if ($myTmpObjectArray['includedocgeneration']) {
		...
		foreach ($dirmodels as $reldir)
		{
			...
			{
				$realpath = $reldir."custom/recufiscal/core/modules/".$valdir;
