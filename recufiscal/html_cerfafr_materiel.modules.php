 ...
 */
class html_cerfafr_materiel extends ModeleDon
{
	/**
	 *  Constructor
	 *
	 ...
	 */
	public function write_file($recufiscal, $outputlangs, $currency = '')
	{
		// phpcs:enable
		global $user, $conf, $langs, $mysoc;

		$now = dol_now();
		$id = (!is_object($recufiscal) ? $recufiscal : '');
		...
		if (!empty($conf->recufiscal->dir_output)) {
			// Definition of the object don (for upward compatibility)
			if (!is_object($recufiscal)) {
				$recufiscal = new RecuFiscal($this->db);
				$ret = $recufiscal->fetch($id);
				$id = $recufiscal->id;
			}
			...
			if (file_exists($dir)) {
				...
				if ($don->mode_reglement_code == 'CHQ') {
					$ModePaiement = '<td width="25%"><input type="checkbox"> Remise d\'espèces</td><td width="25%"><input type="checkbox" checked> Chèque</td><td width="50%"><input type="checkbox"> Virement, prélèvement, carte bancaire</td>';
				} elseif ($don->mode_reglement_code == 'LIQ') {
					$ModePaiement = '<td width="25%"><input type="checkbox" checked> Remise d\'espèces</td><td width="25%"><input type="checkbox"> Chèque</td><td width="50%"><input type="checkbox"> Virement, prélèvement, carte bancaire</td>';
				} elseif ($don->mode_reglement_code == 'VIR' || $don->mode_reglement_code == 'PRE' || $don->mode_reglement_code == 'CB') {
					$ModePaiement = '<td width="25%"><input type="checkbox"> Remise d\'espèces</td><td width="25%"><input type="checkbox"> Chèque</td><td width="50%"><input type="checkbox" checked> Virement, prélèvement, carte bancaire</td>';
				} else {
				...
				*/
				...
				$detailRecu = "SELECT description,valeur,qty,rowid FROM ".MAIN_DB_PREFIX."recu_fiscal_det WHERE fk_recu_fiscal =".$recufiscal->id;
				$resqlDet = $this->db->query($detailRecu);
				$objRecuFiscalDet = $this->db->fetch_object($resqlDet);
				...
				$form = str_replace('__DONATOR_ADDRESS__', $recufiscal->donateur_object->address, $form);
				$form = str_replace('__DONATOR_ZIP__', $recufiscal->donateur_object->zip, $form);
				$form = str_replace('__DONATOR_TOWN__', $recufiscal->donateur_object->town, $form);
				$form = str_replace('__AMOUNT__', $recufiscal->total_ttc, $form);
				$form = str_replace('__AMOUNTLETTERS__', $this->amountToLetters($recufiscal->total_ttc), $form);
