...
class modTest extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		*/
		...
		$this->rights[$r][4] = 'myobject'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update objects of Test'; // Permission label
		$this->rights[$r][4] = 'myobject'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete objects of Test'; // Permission label
		$this->rights[$r][4] = 'myobject'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->test->level1->level2)
		$r++;
		...
		$this->menu[$r++] = array(
			...
			'perms'=>'1', // Use 'perms'=>'$user->rights->test->myobject->read' if you want your menu with a permission rules
			'target'=>'',
		...
		$this->menu[$r++]=array(
			...
			'position'=>1000+$r,
			'enabled'=>'$conf->test->enabled',  // Define condition to show or hide menu entry. Use '$conf->test->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->test->myobject->write',			                // Use 'perms'=>'$user->rights->test->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
	...
	public function init($options = '')
	{
		...
		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			...
			if ($myTmpObjectArray['includerefgeneration']) {
				$src = DOL_DOCUMENT_ROOT.'/install/doctemplates/test/template_myobjects.odt';
				$dirodt = DOL_DATA_ROOT.'/doctemplates/test';
