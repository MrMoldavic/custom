...
/**
 *   	\file       emprunteur_list.php
 *		\ingroup    materiel
 *		\brief      List page for emprunteur
 */
...
// load materiel libraries
require_once __DIR__.'/class/emprunteur.class.php';
...
$hookmanager->initHooks(array('emprunteurlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->materiel->emprunteur->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($conf->materiel->enabled)) accessforbidden('Module not enabled');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Emprunteur';
	$objectlabel = 'Emprunteur';
	$uploaddir = $conf->materiel->dir_output;
...
//$help_url = "EN:Module_Emprunteur|FR:Module_Emprunteur_FR|ES:Módulo_Emprunteur";
$help_url = '';
$title = $langs->trans("Emprunteurs");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/materiel/emprunteur_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/materiel/emprunteur_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendEmprunteurRef";
$modelmail = "emprunteur";
$objecttmp = new Emprunteur($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_materiel', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
