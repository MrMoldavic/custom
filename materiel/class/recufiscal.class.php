...

{
    ...
    */
    ...
    public function getDonationLines()
    {

        // Fetch every source ID of type 'recu_fiscal' from this donor 

        $sql = "SELECT s.rowid FROM ".MAIN_DB_PREFIX."source as s ";

        $sql .= "LEFT JOIN ".MAIN_DB_PREFIX."recu_fiscal as r ON s.fk_source = r.rowid ";

        $sql .= "WHERE s.fk_type_source = 2 AND r.fk_donateur = {$this->id} ";
...

        {
...

            {
                ...
                $source_tmp = new Source($this->db);

                $source_tmp->fetch($obj->rowid);

                foreach($source_tmp->lines as $line)

                {

                    $this->donation_lines[] = $line;
